...
function last_passe_verify(data)
{
  var pass, passconfirm, realpass, bool;
  pass = document.getElementById('passe').value;
  passconfirm = document.getElementById('pass-confirme').value;
  realpass = data;
  if (pass == passconfirm && passconfirm == realpass ) 
  {
  }else{
    document.getElementById('message_modal').innerHTML = "Le mot de passe saisie est incorrecte veuillez réessayer !";
    pasinscrit();
    return false;
  }
 ...
 {
  document.getElementsByClassName('div_profil')[0].style.width = '80%';
  ...
  function course_delete(id)
  {
    var xhr, bool;
     ...
     if (bool == true) 
     {
        xhr.open("get", 'fonctions/course_delete.php?id='+id, true);
        xhr.onreadystatechange = function(){
            if (xhr.readyState == 4 && xhr.status == 200) {
                document.getElementById('centre-accueil').innerHTML = xhr.responseText;
            }
            if(xhr.status == 404)
            {
